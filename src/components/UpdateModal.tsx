...
import { RefreshCw, Download, CheckCircle, AlertCircle, X, ShieldCheck, Sparkles, ArrowRight, Zap, Trash2 } from 'lucide-react';
...
const UpdateModal: React.FC<UpdateModalProps> = ({ isOpen, onClose, lang = 'ro' }) => {
  ...
  // Mod actualizare: 'express' (Păstrează datele / Express) vs 'clean' (Reinstalare curată / Clean reinstall)
      ...
      if ((window as any).api && (window as any).api.checkForUpdates) {
        ...
        setTimeout(() => {
          setUpdateInfo({
            ...
            releaseNotes: [
              lang === 'ro' ? 'Versiunea curentă v1.7.4 este complet la zi.' : 'Current version v1.7.4 is fully up to date.',
  ...
  const handleStartDownload = async () => {
    ...
    try {
      ...
      setDownloading(false);
      setError(err.message || (lang === 'ro' ? 'Eroare la descărcarea actualizării de pe GitHub.' : 'Error downloading update from GitHub.'));
    }
  };
      ...
      <div style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div style={{
                ...
                <div>
                  ...
                  <p style={{ margin: '3px 0 0 0', fontSize: '12.5px', color: '#a7f3d0' }}>
                    {t.updateUpToDateDesc}<strong>v{updateInfo.currentVersion}</strong>.
                  </p>
                </div>
              </div>
              ...
              <div style={{
                ...
                <h5 style={{ margin: '0 0 10px 0', fontSize: '11px', fontWeight: '800', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.8px' }}>
                  {t.updateNotesTitle} (v{updateInfo.currentVersion}):
                </h5>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div style={{
                ...
                <h4 style={{ margin: '10px 0 4px 0', fontSize: '18px', fontWeight: '800', color: '#ffffff' }}>
                  {t.updateAvailableTitle.replace('{ver}', updateInfo.latestVersion)}
                </h4>
                <p style={{ margin: 0, fontSize: '13px', color: '#cbd5e1', display: 'flex', alignItems: 'center', gap: '6px' }}>
                  {t.updateCurrentVer} <strong style={{ color: '#94a3b8' }}>v{updateInfo.currentVersion}</strong>
                  <ArrowRight style={{ width: '14px', height: '14px', color: '#60a5fa' }} />
                  {t.updateNewVer} <strong style={{ color: '#34d399', fontSize: '14px' }}>v{updateInfo.latestVersion}</strong>
                </p>
              </div>
              ...
              <div style={{
                ...
                <h5 style={{ margin: '0 0 8px 0', fontSize: '11px', fontWeight: '800', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.8px' }}>
                  {t.updateNotesTitle}
                </h5>
                <ul style={{ margin: 0, paddingLeft: '18px', fontSize: '12.5px', color: '#cbd5e1', lineHeight: '1.5' }}>
                  {updateInfo.releaseNotes?.map((note, idx) => (
              ...
              {/* Selector Mod de Actualizare (Păstrare Date vs Reinstalare Curată) */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginTop: '4px' }}>
                <label style={{ fontSize: '12px', fontWeight: '800', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.6px' }}>
                  {t.updateSelectModeTitle}
                </label>

                {/* Opțiunea 1: Express Update (RECOMANDAT - Păstrează Datele) */}
                <div
                  ...
                  style={{
                    ...
                    padding: '14px 16px',
                    borderRadius: '14px',
                    backgroundColor: updateMode === 'express' ? 'rgba(16, 185, 129, 0.12)' : 'rgba(255, 255, 255, 0.03)',
                    border: updateMode === 'express' ? '2px solid #10b981' : '1px solid rgba(255, 255, 255, 0.08)',
                    cursor: 'pointer',
                    transition: 'all 0.2s'
                  }}
                >
                  <div style={{
                    ...
                    height: '20px',
                    borderRadius: '50%',
                    border: updateMode === 'express' ? '6px solid #10b981' : '2px solid #64748b',
                    backgroundColor: '#111522',
                    marginTop: '2px',
                    flexShrink: 0
                  }} />
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      ...
                      <strong style={{ fontSize: '13.5px', color: '#ffffff', fontWeight: '800' }}>
                        {t.updateModeExpressTitle}
                      </strong>
                    </div>
                    <p style={{ margin: '4px 0 0 0', fontSize: '12px', color: '#94a3b8', lineHeight: '1.4' }}>
                      {t.updateModeExpressDesc}
                    </p>
                  </div>
                </div>

                {/* Opțiunea 2: Clean Reinstall (Reinstalare curată) */}
                <div
                  ...
                  style={{
                    ...
                    padding: '14px 16px',
                    borderRadius: '14px',
                    backgroundColor: updateMode === 'clean' ? 'rgba(239, 68, 68, 0.12)' : 'rgba(255, 255, 255, 0.03)',
                    border: updateMode === 'clean' ? '2px solid #f87171' : '1px solid rgba(255, 255, 255, 0.08)',
                    cursor: 'pointer',
                    transition: 'all 0.2s'
                  }}
                >
                  <div style={{
                    ...
                    height: '20px',
                    borderRadius: '50%',
                    border: updateMode === 'clean' ? '6px solid #f87171' : '2px solid #64748b',
                    backgroundColor: '#111522',
                    marginTop: '2px',
                    flexShrink: 0
                  }} />
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      ...
                      <strong style={{ fontSize: '13.5px', color: '#ffffff', fontWeight: '800' }}>
                        {t.updateModeCleanTitle}
                      </strong>
                    </div>
                    <p style={{ margin: '4px 0 0 0', fontSize: '12px', color: '#94a3b8', lineHeight: '1.4' }}>
                      {t.updateModeCleanDesc}
                    </p>
                  </div>
                </div>
          ...
          {downloading && !installed && (
            <div style={{ padding: '10px 0' }}>
              ...
              <div style={{
                ...
                <span>{lang === 'ro' ? 'Transferat:' : 'Transferred:'} <strong>{progress.transferred}</strong> / {progress.total}</span>
                <span>{lang === 'ro' ? 'Viteză:' : 'Speed:'} <strong style={{ color: '#60a5fa' }}>{progress.speed}</strong></span>
              </div>
            </div>
          )}
          ...
          {installed && (
            <div style={{
              ...
              <h4 style={{ margin: '0 0 6px 0', fontSize: '18px', fontWeight: '800', color: '#ffffff' }}>
                {lang === 'ro' ? 'Actualizare Descărcată cu Succes!' : 'Update Downloaded Successfully!'}
              </h4>
              <p style={{ margin: 0, fontSize: '13px', color: '#a7f3d0' }}>
                {lang === 'ro'
                  ? `Kit-ul de instalare v${updateInfo?.latestVersion || '1.7.4'} a fost lansat. Aplicația va reporni pentru a finaliza instalarea.`
                  : `Installation setup v${updateInfo?.latestVersion || '1.7.4'} has launched. The app will restart to complete update.`}
              </p>
            </div>
          )}
        ...
        <div style={{
          ...
          {!checking && updateInfo && !updateInfo.hasUpdate && (
            ...
            >
              <RefreshCw style={{ width: '14px', height: '14px' }} /> {t.updateBtnChecking}
            </button>
          )}
          ...
          {!checking && updateInfo && updateInfo.hasUpdate && !downloading && !installed && (
            ...
            >
              <Download style={{ width: '16px', height: '16px' }} /> {t.updateBtnInstall} (v{updateInfo.latestVersion})
            </button>
          )}
          ...
          >
            {installed ? t.updateBtnClose : (updateInfo?.hasUpdate ? t.updateBtnLater : t.updateBtnClose)}
          </button>
        </div>
